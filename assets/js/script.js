...
if (!localStorage.getItem("library")) {
  // If there are no books then let the books be empty
  books = [];
} else {
  //   myLibrary = JSON.parse(localStorage.getItem("library"));
  books = JSON.parse(localStorage.getItem("library"));

  // Load the current books to the library
  books.forEach((book) => {
    addBookToLibrary(
      book.cover,
      book.title,
...
function Book(cover, title, author, pageCount, status = false) {
  ...
  this.info = () => {
    infoString = `${this.title} by ${this.author}, ${this.pageCount} pages, ${
      this.readStatus ? "not read yet" : "finished reading"
    }`;
...
function handleSubmit(e) {
  ...
  bookData.readStatus = bookData.readStatus ? true : false;
  books.push(bookData);
